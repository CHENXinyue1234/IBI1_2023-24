...
city_names_UK=['Stirling','Edinburgh','Glasgow','London'] #list the cities to which the population corresponds
width = 0.8 #set the width
plt.figure() #create a new figure
plt.bar(city_names_UK,uk_cities, width,color='g')#create a bar graph and set the color green
for i in range(len(city_names_UK)):
...
plt.xticks(city_names_UK)#set x labels
plt.show()#show the figure
plt.clf() #close the figure

#Draw a bar graph for China cities
city_names_China=['Haining','Hangzhou','Beijing','Shanghai']
width = 0.8
plt.figure()
plt.bar(city_names_China,China_cities, width,color='b')#create a bar graph and set the color blue
for i in range(len(city_names_China)):
    plt.text(i,China_cities[i],str(China_cities[i]),ha='center',va='bottom')
plt.ylabel("Population(millions)")
plt.title("China cities",fontsize=20)
plt.xticks(city_names_China)
plt.show()
plt.clf() 
